  ...
  return (
    <div className="min-h-screen dark-theme content-offset">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {/* TITLE */}
        <div className="soft-card p-8 sm:p-12 card-appear mb-6">
          <h1 className="text-center custom-heading">
            My <span style={{ color: "#14b8a6" }}>Account</span>
          </h1>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 sm:gap-10">
          ...
          <div className="lg:col-span-1">
            <div className="rounded-xl p-2" style={{ backgroundColor: '#0f172a', border: '1px solid #334155' }}>
              {tabs.map((t) => (
                <button
                  key={t.id}
                  onClick={() => setActiveTab(t.id)}
                  className={`w-full flex items-center gap-3 px-6 py-3 rounded-xl mb-3 transition account-tab ${activeTab === t.id ? '' : ''}`}
                  style={{
                    backgroundColor: activeTab === t.id ? '#14b8a6' : 'transparent',
                    color: activeTab === t.id ? '#ffffff' : '#e5e7eb',
                    border: activeTab === t.id ? '1px solid #14b8a6' : '1px solid #334155'
                  }}
                >
                  <span style={{ color: activeTab === t.id ? '#ffffff' : '#94a3b8' }}>{t.icon}</span>
          ...
          <div className="lg:col-span-3">
            ...
            {/* PROFILE TAB */}
            {activeTab === 'profile' && (
              <div className="rounded-2xl p-8" style={{ backgroundColor: '#0f172a', border: '1px solid #334155' }}>
                <h2 className="custom-heading mb-6" style={{ color: '#e5e7eb' }}>Profile Information</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {[
                    ...
                    <div key={i}>
                      <label className="block mb-2 font-medium" style={{ color: '#cbd5e1' }}>
                        {item.label}
                      </label>
                      <input
                        value={item.value}
            ...
            {activeTab === 'orders' && (
              <div className="rounded-2xl p-8" style={{ backgroundColor: '#0f172a', border: '1px solid #334155' }}>
                <h2 className="custom-heading mb-6" style={{ color: '#e5e7eb' }}>My Orders</h2>
                    ...
                    {orders.map((order) => (
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="order-item-title text-lg" style={{ color: '#e5e7eb' }}>
                              Order #{order.id}
                            </p>
                            <p className="order-item-meta mt-1" style={{ color: '#94a3b8' }}>
                              {new Date(order.createdAt).toLocaleDateString()}
                            </p>
                          </div>

                          <div className="text-right">
                            <p className="order-price text-lg" style={{ color: '#e5e7eb' }}>₹{order.total.toFixed(2)}</p>
                            <p className="order-status capitalize mt-1" style={{ color: '#cbd5e1' }}>
                              {order.status}
                            </p>
                          </div>
                        </div>
            ...
            {activeTab === 'wishlist' && (
              <div className="rounded-2xl p-8" style={{ backgroundColor: '#0f172a', border: '1px solid #334155' }}>
                <h2 className="custom-heading mb-6" style={{ color: '#e5e7eb' }}>Wishlist</h2>
                    ...
                    {wishlistProducts.map((p) => (
                      <a key={p.id} href={`/product/${p.id}`} className="group bg-white rounded-xl shadow-sm hover:shadow-lg transition overflow-hidden">
                        ...
                        <div className="p-4">
                          ...
                          <p className="font-semibold" style={{ color: '#0f172a' }}>₹ {Number(p.price).toFixed(2)}</p>
                        </div>
                      </a>
                    ))}
                  </div>
                ) : (
                  <p className="text-center py-12" style={{ color: '#cbd5e1' }}>
